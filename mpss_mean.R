...
for (f in f.mpss){
    ...
    t.mpss.prc <- t.mpss %>%
    mutate(datetime = as.POSIXct("2023-01-01 00:00:00 UTC",tz="UTC") + date*86400)%>%select(-date)%>% # change year to your needs
    ...
    data.table::fwrite(x=t.mpss.prc, file = paste0("output/",strsplit(basename(f),".in2")[1],"_1hmean.in2"),
    col.names = T, row.names = F, append=F, quote=F)
}
